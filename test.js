...
describe("formatLocaleCurrency", () => {
  it("abbreviates thousands", () => {
    let result = formatLocaleCurrency(3000.54, "USD", "US", true);
    chai.expect(result).to.equal("$3k");
  });

  it("abbreviates millions", () => {
    let result = formatLocaleCurrency(3430000.54, "USD", "US", true);
    chai.expect(result).to.equal("$3.4m");
  });

  it("abbreviates billions", () => {
    let result = formatLocaleCurrency(8100000000.54, "USD", "US", true);
    chai.expect(result).to.equal("$8.1b");
  });

  it("handles abbreviation when less than a thousand", () => {
    let result = formatLocaleCurrency(10.54, "USD", "US", true);
    chai.expect(result).to.equal("$10.54");
  });
});
